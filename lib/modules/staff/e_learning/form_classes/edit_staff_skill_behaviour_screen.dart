...
class _EditSkillsBehaviourScreenState extends State<EditSkillsBehaviourScreen> {
  ...
  void _saveChanges() {
    ...
    final updatedSkills = <int, Map<int, String>>{};

    _controllers.forEach((studentIndex, skillControllers) {
      final student = provider.students[studentIndex];
      final skillMap = <int, String>{};
      skillControllers.forEach((skillId, controller) {
        if (controller.text.isNotEmpty) {
          skillMap[skillId] = controller.text;
        }
      });
      updatedSkills[student.id] = skillMap;
    });

    // Call provider method to save changes (assuming such a method exists)
    // provider.updateSkillsAndBehaviours(
    //   classId: widget.classId,
    //   levelId: widget.levelId,
    //   term: widget.term,
    //   year: widget.year,
