...
class StaffViewQuestionScreen extends StatefulWidget {
  ...
  @override
  State<StaffViewQuestionScreen> createState() => _StaffViewQuestionScreenState();
}

class _StaffViewQuestionScreenState extends State<StaffViewQuestionScreen> {
...
void _initializeQuestions() {
  ...
  createdQuestions = widget.questions!.map((q) {
    ...
    print('SSSSSSSS${currentQuestion.duration}SSSSSSSSSSSSSSSSSSSSSSS$id');
    ...
    if (questionType == 'multiple_choice') {
      ...
      for (int i = 0; i < options.length; i++) {
        final opt = options[i];
        final optText = (opt is Map && opt['text'] != null) ? opt['text'].toString() : '';
        
        optionControllers.add(TextEditingController(text: optText));
        
        // Handle option files
        Map<String, dynamic>? optionFile;
        if (opt is Map && opt['option_files'] is List && (opt['option_files'] as List).isNotEmpty) {
      ...
      if (correct is Map && correct['order'] != null) {
        final correctOrder = correct['order'] is int ? correct['order'] : int.tryParse(correct['order'].toString()) ?? -1;
      ...
      if (correct is Map && correct['text'] != null) {
        correctAnswerController = TextEditingController(text: correct['text'].toString());
      } else {
  ...
  setState(() {
    ...
    for (var question in createdQuestions) {
      ...
      updatedQuestions.add({
        ...
        'grade': marksController.text.isNotEmpty ? marksController.text : '',
        ...
        GestureDetector(
          ...
          child: ListTile(
            ...
            subtitle: Text(
              ...
              style: AppTextStyles.normal400(fontSize: 1, color: AppColors.textGray),
            ),
            trailing: IconButton(
...
// Method to save questions to SharedPreferences
Future<void> _saveQuestionsToPrefs() async {
  final prefs = await SharedPreferences.getInstance();
  // Clear existing questions in SharedPreferences
  await prefs.remove('preview_questions');
  // Prepare questions data for serialization
  final questionsToSave = createdQuestions.map((q) {
    return {
      'question_id': q['question_id']?.toString() ?? '', // Always include question_id
      'question_text': q['title'] ?? '',
      'question_type': q['type'] ?? '',
      'question_grade': q['grade']?.toString() ?? '1',
      'topic': q['topic'] ?? '',
      'options': q['options'] ?? [],
      'correct': q['correct'] ?? [],
      'question_files': q['imagePath'] != null
  ...
  print('Saving questions to SharedPreferences: $questionsToSave');
 ...
 void _addQuestion(String questionType) {
  ...
  setState(() {
    ...
    createdQuestions.add({
      ...
      'options': questionType == 'multiple_choice'
          ...
          : [],
      'correct': questionType == 'multiple_choice' ? [] : [{'order': 0, 'text': ''}],
      'imagePath': null,
      'imageName': null,
      'question_id': '', // Explicitly set for new questions
      'questionController': questionController,
      'marksController': marksController,
      'optionControllers': optionControllers,
      'correctOptions': correctOptions,
  ...
  // IMPORTANT: Save to SharedPreferences immediately after adding
  _saveCurrentQuestionsToPrefs();
}
...
Future<void> _saveCurrentQuestionsToPrefs() async {
  ...
  final questionsToSave = createdQuestions.map((q) {
    ...
    return {
      ...
      'question_text': questionController.text.isNotEmpty ? questionController.text : 'Untitled Question',
      'question_type': q['type'] ?? '',
      'question_grade': marksController.text.isNotEmpty ? marksController.text : '1',
      'topic': q['topic'] ?? '',
      'options': q['type'] == 'multiple_choice'
          ? optionControllers.asMap().entries.map((e) {
              final optionsData = q['options'] as List<Map<String, dynamic>>;
              final optionFile = optionsData.length > e.key ? optionsData[e.key]['options_file'] : null;
              return {
                'order': e.key,
                'text': e.value.text,
                'option_files': optionFile != null
      ...
      'correct': q['type'] == 'multiple_choice'
          ? (q['correctOptions'] as List<int>).isNotEmpty
              ? {
                  'order': (q['correctOptions'] as List<int>).first,
                  'text': optionControllers.isNotEmpty && (q['correctOptions'] as List<int>).first < optionControllers.length
                      ? optionControllers[(q['correctOptions'] as List<int>).first].text
                      : '',
                }
              : {'order': 0, 'text': ''}
          : {
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text(
                    (questionController.text.isEmpty ? 'Untitled Question' : questionController.text[0].toUpperCase() + questionController.text.substring(1)),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                // Replace your existing question TextField in _buildQuestionCard with this:
TextField(
  ...
  onChanged: (value) {
    // Auto-save to SharedPreferences when text changes
    if (index != -1) {
                    ...
                    Column(
                      children: [
                        ...optionControllers.asMap().entries.map((entry) {
                          ...
                          final hasImage = index != -1 && createdQuestions[index]['options'][optionIndex]['options_file'] != null;
...
void _deleteQuestionFromList(int questionIndex) async {
    ...
    try {
      ...
      CustomToaster.toastError(context, "Error", " Error deleting questions: $e");
    }
  }
// In your _buildOptionRow method, modify the onChanged callback:
  ...
  return Row(
    children: [
      Radio<bool>(
        ...
        onChanged: (value) {
          onSelectCorrect();
          // Save to SharedPreferences when correct option changes
          _saveCurrentQuestionsToPrefs();
        },
      ),
      Expanded(
        child: TextField(
          ...
          onChanged: (value) {
            ...
            // Auto-save to SharedPreferences when option text changes
            _saveCurrentQuestionsToPrefs();
          },
        ),
      ),
