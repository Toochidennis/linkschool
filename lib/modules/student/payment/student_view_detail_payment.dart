  ...
  try {
    if (_emailController.text.isNotEmpty &&
      ...
      if (_calculateSelectedTotal() == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please select at least one fee.')),
        );
        return;
      }
      ...
      // Prepare the data exactly as in your original code
      final data = {
        'email': _emailController.text,
        'amount': _calculateSelectedTotal().toInt(),
        "invoice_id": widget.invoice.id.toString(),
        "reference": widget.invoice.reference,
        "reg_no": widget.invoice.regNo,
        "name": user['name'],
        "fees": widget.invoice.details
            ...
            .map((entry) => {
                 ...
                 'fee_name': entry.value.feeId,
                 'amount': entry.value.feeAmount,
               })
            .toList(),
      ...
      // Await the payment initialization
      await paymentProvider.initializePayment(
        invoiceId: widget.invoice.id.toString(), 
        reference: widget.invoice.reference, 
        regNo: widget.invoice.regNo, 
        name: user['name'], 
        amount: _calculateSelectedTotal(), 
        fees: widget.invoice.details
            .asMap()
            .entries
            .where((entry) => _selectedFees[entry.key])
      ...
      // If you still need to call the direct Paystack API, await it too
    ...
    Future<void> initializePayment(
      ...
      if (response.statusCode == 200) {
        ...
        //       reference: '',
        //       dbName: 'aalmgzmy_linkskoo_practice', // Replace with the appropriate dbName value
        //     ),
        //   ),
        // );
      } else {
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: _hasSelection
                  ? const Color(0xFF2F55DD) // blue when active
