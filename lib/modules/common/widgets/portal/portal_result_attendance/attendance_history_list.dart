...
class AttendanceHistoryList extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: ListView.separated(
          ...
          separatorBuilder: (context, index) => Divider(
              height: 1, color: Colors.grey[300], indent: 16, endIndent: 16),
          itemBuilder: (context, index) =>
              _buildAttendanceHistoryItem(context, index),
        ),
      ),
    );
  }
  ...
  Widget _buildAttendanceHistoryItem(BuildContext context, int index) {
    return ListTile(
      ...
      subtitle:
          Text(subjects[index], style: const TextStyle(color: Colors.grey)),
      onTap: () => Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  AttendanceHistoryScreen(date: dates[index]))),
    );
  }
}
