...
class _StartChallengeState extends State<StartChallenge>
  ...
  void initState() {
    ...
    if (widget.examIds != null && widget.subjectNames != null && widget.years != null) {
  ...
  Future<void> _loadQuestions() async {
    if (widget.examIds == null || widget.examIds!.isEmpty || currentExamIndex >= widget.examIds!.length) return;

    final provider = context.read<ExamProvider>();
    final currentExamId = widget.examIds![currentExamIndex];

    try {
      await provider.fetchExamData(currentExamId, limit: widget.questionLimit ?? 40);
      if (mounted) {
        setState(() {});
        Navigator.of(context).pop(); // Pop the countdown dialog
      }
    } catch (e) {
      print("Load error: $e");
      if (mounted) {
        setState(() {});
        Navigator.of(context).pop(); // Pop the countdown dialog on error
      }
    }
  }
  ...
  void _showLoadingCountdown() {
    showDialog(
      ...
      builder: (_) => _LoadingCountdownDialog(onComplete: () {}),
    );
    _loadQuestions();
  }
  ...
  void _selectAnswer(int optionIndex) {
    ...
    final correctAnswerOrder = provider.questions[currentIdx].correctAnswer?['order'];
    final isCorrect = optionIndex == correctAnswerOrder;
    print('Answer Selected: ${isCorrect ? "Correct" : "Wrong"} (Selected: $optionIndex, Correct: $correctAnswerOrder)');
    ...
    // Note: User can now close popup manually, so we don't auto-advance
    // Popup will handle the advancement when closed
  }
  ...
  void _completeCurrentExam(ExamProvider provider) {
  ...
  void _showFinalResults() {
    ...
    for (var entry in allAnswers.entries) {
      ...
      if (index >= 0) {
        print('   ${widget.subjectNames![index]}: ${answers.length}/${questions.length} answered');
      }
    }
  ...
  int _calculateScore(ExamProvider p) {
    int score = 0;
  ...
  int _calculateCorrectAnswers(ExamProvider p) {
    int correct = 0;
    for (int i = 0; i < p.questions.length; i++) {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        ...
                        SizedBox(height: 30),
                        if (question != null) _buildOptions(question, selected),
                        SizedBox(height: 100),
                      ],
                    ),
                  ),
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 20),
              Text("Loading Challenge...", style: AppTextStyles.normal600(fontSize: 16)),
            ],
          ),
        ),
      );
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text("Failed to load questions", style: AppTextStyles.normal600(fontSize: 18)),
  ...
  Widget _buildHeader(ExamProvider p, double progress) {
    return Container(
      padding: EdgeInsets.fromLTRB(20, 20, 20, 16),
      decoration: BoxDecoration(
        gradient: LinearGradient(begin: Alignment.topLeft, colors: [Colors.blue.shade50, Colors.white]),
      ...
      child: Column(
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.challenge?.title ?? "CBT Challenge", style: AppTextStyles.normal600(fontSize: 20)),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [Colors.red.shade500, Colors.orange.shade600]),
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 6),
                    Text(_formatTime(_remainingSeconds ?? 0), style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            ...
            children: [
              Text("Question ${p.currentQuestionIndex + 1} of ${p.questions.length}", style: TextStyle(fontWeight: FontWeight.w600)),
         ...
         constraints: BoxConstraints(maxWidth: 600),
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                Colors.white,
                isPerfect ? Colors.amber.shade50 : 
                isGood ? Colors.green.shade50 : 
                Colors.blue.shade50,
              ],
            ),
              ...
              builder: (context, value, child) {
                return CircularProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation<Color>(
                    isPerfect ? Colors.amber.shade500 : 
                    isGood ? Colors.green.shade500 : 
                ...
                builder: (context, value, child) {
                  return Text(
                    ...
                    style: TextStyle(
                      ...
                      color: isPerfect ? Colors.amber.shade700 : 
                             isGood ? Colors.green.shade700 : 
...
class _FloatingStarsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      _drawStar(canvas, Offset(x, y), 8 + math.sin(progress * math.pi * 2) * 2, paint);
    }
  }
