...
class _AssessmentScreenState extends State<AssessmentScreen> {
  ...
  // Fixed _loadQuestions method for AssessmentScreen
...
// Helper method to process question data
  QuizQuestion _processQuestionData(Map<String, dynamic> q, int index) {
    ...
    final String questionText =
        q['question_text']?.toString() ?? 'Question ${index + 1}';
    ...
    final String questionType =
        q['question_type']?.toString() ?? 'short_answer';

    if (questionType == 'multiple_choice') {
      return _createMultipleChoiceQuestion(
          q, topic, questionText, questionGrade, imagePath);
    } else {
      return _createShortAnswerQuestion(
          q, topic, questionText, questionGrade, imagePath);
    }
  }
...
// Helper method to check if string is base64
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              value:
                  _totalQuestions > 0 ? answeredQuestions / _totalQuestions : 0,
          ...
          children: [
            ...
            Text(
              question.questionText.isNotEmpty
                  ? question.questionText[0].toUpperCase() +
                      question.questionText.substring(1)
                  : "Question",
          ...
          return Container(
            ...
            child: RadioListTile<String>(
              title: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (!isImage) Text(option['text']),
                  if (isImage)
                    GestureDetector(
                      onTap: () =>
                          _showFullScreenOptionImage(option['imageUrl']),
                      child: SizedBox(
                        width: 400,
                        height: 100,
                        child: _buildOptionImage(option['imageUrl']),
                      ),
                    ),
                ],
              ),
  ...
  Widget _buildOptionImage(String imageUrl) {
    if (imageUrl.startsWith('data:')) {
      ...
      return Image.memory(
        base64Decode(base64String),
        fit: BoxFit.cover,
        errorBuilder: (context, error, stackTrace) => _buildOptionErrorImage(),
      );
    } else {
      // Network image
      return Image.network(
  ...
  void _showFullScreenOptionImage(String imageUrl) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => Scaffold(
      ...
      if (question is TextQuestion) {
        ...
        if (correctAnswer != null) {
          ...
          bool endsWithImageExtension =
              correctAnswer.toLowerCase().endsWith('.jpg') ||
            ...
            for (var option in question.options) {
              if (option['text'] == correctAnswer &&
                ...
                processedCorrectAnswers.add(
                    {'text': correctAnswer, 'imageUrl': option['imageUrl']});
                foundMatch = true;
                break;
              }
            }
            if (!foundMatch) {
              processedCorrectAnswers
                  .add({'text': correctAnswer, 'imageUrl': null});
            }
          } else {
            processedCorrectAnswers
                .add({'text': correctAnswer, 'imageUrl': null});
          ...
          processedCorrectAnswers.add({'text': '', 'imageUrl': null});
        }
      } else if (question is TypedAnswerQuestion) {
